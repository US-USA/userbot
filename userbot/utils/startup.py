...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await us7a5.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/e765806fb50294079a58c.mp4", caption="♛ ⦙ تـمّ اعـادة تشـغيل\n تليثـون العـرب ✓  :  [ 7.5 ] .\n\n♛ ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n♛ ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n♛ ⦙ القناة الرسمية تليثون العرب : @us7a5\n♛ ⦙ فارات سورس تليثون  :@TEAMTELETHON \n♛ ⦙ كلايش تليثون :  @FGFFG\n ♛ ⦙التحديثات والاضافات :  @M4_STORY\n",                buttons=[(Button.url("مطور تليثون الرسمي", "https://t.me/lll5l"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
async def add_bot_to_logger_group(chat_id):
    bot_details = await us7a5.tgbot.get_me()
    try:
        await us7a5(            functions.messages.AddChatUserRequest(                chat_id=chat_id,                user_id=bot_details.username,                fwd_limit=1000000            )        )
...
async def setup_bot():
    try:
        await us7a5.connect()
        config = await us7a5(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == us7a5.session.server_address:
                if us7a5.session.dc_id != option.id:
                    LOGS.warning(                        f"♛ ︙ معرف DC ثابت في الجلسة من {us7a5.session.dc_id}"                        f"♛ ︙ يتبع ل {option.id}"                    )
                us7a5.session.set_dc(option.id, option.ip_address, option.port)
                us7a5.session.save()
                break
        bot_details = await us7a5.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await us7a5.start(bot_token=Config.TG_BOT_USERNAME)
        us7a5.me = await us7a5.get_me()
        us7a5.uid = us7a5.tgbot.uid = utils.get_peer_id(us7a5.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(us7a5.me)
    except Exception as e:
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await us7a5.get_entity(BOTLOG_CHATID)
        ...
        _, groupid = await create_supergroup(            "مجموعه بوت تليثون الخاص بك", us7a5, Config.TG_BOT_USERNAME, descript        )
        addgvar("PRIVATE_GROUP_BOT_API_ID", groupid)
        print("♛ ︙ تم إنشاء مجموعة المسـاعدة بنجاح وإضافتها إلى المتغيرات.")
        flag = True
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await us7a5.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
        ...
        _, groupid = await create_supergroup(            "كـروب تخزين الخاص", us7a5, Config.TG_BOT_USERNAME, descript        )
