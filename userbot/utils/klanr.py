...
def remove_plugin(shortname):
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(us7a5._event_builders))):
            ev, cb = us7a5._event_builders[i]
            if cb.__module__ == name:
                del us7a5._event_builders[i]
    except BaseException:
        raise ValueError
